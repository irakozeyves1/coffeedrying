...
<div style="width:800px; height:330px; background:#99CCFF; margin:0px auto 0px auto"><h1 align="center"; style="color:#663333; size:2">Coffee Before drying Report</h1>
<?php include('conection.php'); ?>

<body>
               <center> <form method="post" action="befdryingsearch.php">
...
<div style=" max-height:200px; max-width:500px; overflow:scroll">
              ...
              <center>  <table cellpadding="2" cellspacing="2" border="2" bordercolor="#000000" class="table table-striped table-bordered" >
                    ...
                    <thead >
                         <tr> <th>id</th><th>date</th><th>Product</th><th>Quantity</th><th>UPDATE</th></tr>
                    </thead>
                    <tbody>
                        <?php
                        $query =mysql_query("select b.id_bdry,b.date,p.pro_name,b.qty from bdrying as b,product as p where b.p_id=p.p_id")or die(mysql_error());
				...
				<?php
                echo '<td>' . $row['id_bdry'] . '</td>';
				echo '<td>' . $row['date'] . '</td>';
				echo '<td>' . $row['pro_name'] . '</td>';
				echo '<td>' . $row['qty'] . '</td>';
				
				
	 echo"<td ><a href ='befdryingup.php?id_bdry=$id'>";?><button type="button" name="Submit" onClick="return confirm('You Are Going to Update Record!!')">update</button><?php print"</a></td>";
