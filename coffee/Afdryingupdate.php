...
<div style="width:800px; height:330px; background:#99CCFF; margin:0px auto 0px auto"><h1 align="center"; style="color:#663333; size:2">Coffee After drying Report</h1>
<?php include('conection.php'); ?>

<body>
               <center> <form method="post" action="Afdryingsearch.php">
...
<div style=" max-height:200px; max-width:500px; overflow:scroll">
              ...
              <center>  <table cellpadding="2" cellspacing="2" border="2" bordercolor="#000000" class="table table-striped table-bordered" >
                    ...
                    <tbody>
                        <?php
                        $query =mysql_query("select a.adry_id,a.date,p.pro_name,a.qty from adrying as a,product as p where a.p_id=p.p_id")or die(mysql_error());
				...
				<?php
                echo '<td>' . $row['adry_id'] . '</td>';
				echo '<td>' . $row['date'] . '</td>';
				echo '<td>' . $row['pro_name'] . '</td>';
				echo '<td>' . $row['qty'] . '</td>';
				
				
	 echo"<td ><a href ='Afdryingup.php?adry_id=$id'>";?><button type="button" name="Submit" onClick="return confirm('You Are Going to Update Record!!')">update</button><?php print"</a></td>";
